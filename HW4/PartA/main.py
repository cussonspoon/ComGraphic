...
def draw_sphere(x, y, z, r, transparent=False):
    glPushMatrix()
    glTranslatef(x, y, z)
    ...
    if transparent:
        # Reliable transparency in fixed pipeline:
        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glDepthMask(GL_FALSE)  # do not write depth for transparent object

        glDisable(GL_COLOR_MATERIAL)
        glColor4f(0.7, 0.7, 1.0, 0.30)
        glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (0.7, 0.7, 1.0, 0.30))
        glMaterialfv(GL_FRONT, GL_SPECULAR, (0.6, 0.6, 0.7, 0.30))
        glMaterialf(GL_FRONT, GL_SHININESS, 30.0)
    else:
        glDisable(GL_BLEND)
        glDepthMask(GL_TRUE)

        glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, (0.0, 1.0, 1.0, 1.0))
        glMaterialfv(GL_FRONT, GL_SPECULAR, (1.0, 1.0, 1.0, 1.0))
        glMaterialf(GL_FRONT, GL_SHININESS, 60.0)

    gluSphere(quad, r, 48, 48)
...
def main():
    ...
    pygame.display.set_caption("Demo: Textured Floor + Transparent Sphere + OpenGL Text Overlay")

    overlay_lines = [
        "DEMO: PyGame + OpenGL (Fixed Pipeline)",
        "Floor texture: Escher-style bird tessellation (chess.png) | GL_REPEAT tiling",
        "Objects: Left sphere = opaque | Right sphere = transparent (alpha blending)",
    ...
    # Load floor texture (your bird tessellation)
    tex = load_texture("chess.png")

    # Pre-create text textures once (fast and reliable)
    overlay_tex = []
    ...
    try:
        while running:
            ...
            # Update light position after camera is applied (modelview matters)
            glLightfv(GL_LIGHT0, GL_POSITION, light_pos)

            # 3D scene
            draw_floor(tex)
            draw_sphere(-3.0, 2.0, 0.0, 2.0, transparent=False)
            draw_sphere( 3.0, 2.0, 0.0, 2.0, transparent=True)
